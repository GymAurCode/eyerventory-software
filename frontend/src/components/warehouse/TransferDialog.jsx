...
function emptyRow() { return { product_id: "", quantity: 1, notes: "" }; }

export default function TransferDialog({ open, onClose, onSaved }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await warehouseApi.transfer(payload, {
        source_warehouse_id: Number(sourceId),
        dest_warehouse_id: Number(destId),
  ...
  return (
    <Modal title="Transfer Stock" open={open} onClose={onClose} maxWidth="max-w-3xl">
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="flex gap-2">
          <select className="input py-2 flex-1" value={sourceId}
            onChange={(e) => setSourceId(e.target.value)} required>
            <option value="">Source Warehouse</option>
            {warehouses.map((w) => <option key={w.id} value={w.id}>{w.name}</option>)}
          </select>
          <select className="input py-2 flex-1" value={destId}
        ...
        <input className="input py-2 w-full" value={referenceNo} placeholder="Reference No"
        ...
        <input className="input py-2 w-full" value={notes} placeholder="Notes"
