...
export default function StockOutDialog({ open, onClose, onSaved }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await warehouseApi.stockOut(Number(warehouseId), payload, {
      ...
      toast.error(err.response?.data?.detail || "Stock-out failed");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <Modal title="Stock Out" open={open} onClose={onClose} maxWidth="max-w-3xl">
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="flex gap-2">
          <select className="input py-2 flex-1" value={warehouseId}
            onChange={(e) => setWarehouseId(e.target.value)} required>
            <option value="">Select Warehouse</option>
            {warehouses.map((w) => <option key={w.id} value={w.id}>{w.name}</option>)}
          </select>
          <input className="input py-2 flex-1" value={referenceNo} placeholder="Reference No"
